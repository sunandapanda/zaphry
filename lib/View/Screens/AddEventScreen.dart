...
class _AddEventScreenState extends State<AddEventScreen> {
  ...
  bool attachmentsAttached = false;
        ...
        if (isStartDateSelected) {
          if (isStartTimeSelected) {
            if (bannerAttached) {
              ...
              Controller.showSnackBar("Please add a banner image", context);
            }
          } else {
  ...
  void saveOnPressed() async {
    ...
    if (check) {
      EasyLoading.show();
      String response = await APICalls.addEvent(
          ...
          sendAttachments: attachmentsAttached);
      EasyLoading.dismiss();
      Controller.showSnackBar(response, context);
      if (response == "Successfully Created Event") {
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: ElevatedButton(
                        ...
                        if (attachments != null) {
                          if (attachments.length > 0) {
                            ...
                            attachments.forEach((element) {
                              attachmentsPaths.add(element.path);
                            });
                            setState(() {
                      ...
                      child: Text(
                        attachmentsPaths.length > 0
                            ? "${attachmentsPaths.length} images"
                            : "Pick images",
                        style: TextStyle(
                          color: Colors.white,
                        ),
