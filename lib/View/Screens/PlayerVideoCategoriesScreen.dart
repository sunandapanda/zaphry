...
class PlayerVideoCategoriesScreen extends StatefulWidget {
  const PlayerVideoCategoriesScreen({Key? key}) : super(key: key);

  @override
  _PlayerVideoCategoriesScreenState createState() =>
      _PlayerVideoCategoriesScreenState();
}

class _PlayerVideoCategoriesScreenState
    extends State<PlayerVideoCategoriesScreen> {
  ...
  Future<void> getVideos({int? index}) async {
    EasyLoading.show();
    String videoResponse = await APICalls.getPlayerVideos(
        index == null
  ...
  Widget getVideoItem(Video video) {
      ...
      child: Controller.userCoachVideos.length > 0
          ? GridView.count(
              ...
              children: Controller.userCoachVideos
                  .map(
                    (coachVideo) => getVideoItem(coachVideo),
                  )
                  .toList(),
            )
          : Center(
