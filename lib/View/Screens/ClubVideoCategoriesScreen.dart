...
class ClubVideoCategoriesScreen extends StatefulWidget {
  const ClubVideoCategoriesScreen({Key? key}) : super(key: key);

  @override
  _ClubVideoCategoriesScreenState createState() =>
      _ClubVideoCategoriesScreenState();
}

class _ClubVideoCategoriesScreenState extends State<ClubVideoCategoriesScreen> {
  ...
  Future<void> getVideos({int? index}) async {
    EasyLoading.show();
    String videoResponse = await APICalls.getClubVideos(
        index == null
  ...
  Widget getVideoItem(Video clubVideo) {
    return GestureDetector(
      child: Padding(
        ...
        child: Container(
          ...
          child: ClipRRect(
            ...
            child: Image.network(clubVideo.image!, fit: BoxFit.cover),
          ),
        ),
      ),
      onTap: () async {
        EasyLoading.show();
        String response = await APICalls.getVideoDetails(clubVideo.id);
        EasyLoading.dismiss();
        if (response == "Successfully Retrieved Video Details") {
          if (Controller.videoDetails != null) {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => VideoPlayerScreen(allowEdit: true),
              ),
            );
          } else {
      ...
      onLongPress: () async {
        ...
        if (result) {
          EasyLoading.show();
          String response = await APICalls.deleteVideo(clubVideo.id);
          EasyLoading.dismiss();
          if (response == "Successfully Deleted Video") {
            int itemIndex = Controller.clubVideos
                .indexWhere((element) => element.id == clubVideo.id);
            if (itemIndex >= 0) {
              setState(() {
                Controller.clubVideos.removeAt(itemIndex);
              });
            }
          }
          Controller.showSnackBar(response, context);
  ...
  Widget getVideosContainer() {
    return Container(
      ...
      child: Controller.clubVideos.length > 0
          ? GridView.count(
              controller: scrollController,
              crossAxisCount: 3,
              children: Controller.clubVideos
                  .map(
                    (clubVideo) => getVideoItem(clubVideo),
                  )
                  .toList(),
            )
          : Center(
          ...
          onPressed: () async {
            if (!showError) {
              bool response = await showDialog(
                context: context,
                builder: (BuildContext context) {
                  return AddVideoDialog(isCoach: false);
                },
              );
              if (response) {
                pageNumber = 1;
